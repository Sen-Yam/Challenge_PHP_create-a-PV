<?php
    if(isset($_POST['date']) && isset($_POST['heure1']) && isset($_POST['heure2'])) {

        include'connection1.php';
        $d1=$_POST['date'];
        $h1=$_POST['heure1'];
        $h2=$_POST['heure2'];
        $sql="INSERT INTO test2 VALUES('$d1' , '$h1' , '$h2')";
...
<body style="background-color: rgb(231, 248, 243);">
    ...
    <h1>Entrer les personnes presentes dans la reunion</h1>
    <div class="perso" >
        <button id="h">Ajouter une personne</button>
       <form action="tryphp3.php" class="f" method="post">
        <button class="btn" name="boutton">Continuer</button>
        <div id="i1">
            <div style=" margin-top: 40px; ">
                <input type="text" placeholder="Nom & Prenom P" name="person[]">
            </div>
...
<script>
        document.querySelector('#h').onclick= function(e) {
 		...
 		newspace.setAttribute("placeholder" , "Nom & Prenom P");
 		newspace.setAttribute("type" , "text");
 		newspace.classList.add('oui'); 
 		newspace.setAttribute("name" , "person[]");
